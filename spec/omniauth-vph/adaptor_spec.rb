...
describe OmniAuth::Vph::Adaptor do
  ...
  describe '#user_info' do
    before(:each) do
      ...
      allow(@faraday).to receive(:get)
        .with('/validatetkt/', ticket: 'correct_ticket')
        .and_return(faraday_response(200, '{"user": "details"}'))

      allow(@faraday).to receive(:get)
        .with('/validatetkt/', ticket: 'wrong_ticket')
        .and_return(faraday_response 403)

      allow(@faraday).to receive(:get)
        .with('/validatetkt/', ticket: 'wrong_host')
    ...
    it 'throws ConnectionError when unable to connect to the service' do
      ...
      end.to raise_error(OmniAuth::Vph::Adaptor::ConnectionError)
    end
  end
  ...
  describe '#map_user' do
    ...
    it 'returns user info' do
      ...
      expect(user_map['roles']).to eq %w(admin developer)
    end
  end
